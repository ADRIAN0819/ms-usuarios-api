...
module.exports.modificarProducto = async (event) => {
  ...
  try {
    ...
    if (!validacion.ok) {
      const errorResponse = validacion.respuesta;
      errorResponse.headers = corsHeaders;
      return errorResponse;
    }

    const userId = validacion.datos.user_id;
    const { codigo, nombre, descripcion, precio, cantidad, imagen_base64 } =
    ...
    if (!resultado.Item) {
      return responseWithCors(404, { msg: "Producto no encontrado" });
    }
    ...
    return responseWithCors(200, { msg: "Producto actualizado exitosamente" });
  } catch (error) {
